...
function renderGroup(envelopes = [createMockEnvelope()]) {
  const onEdit = vi.fn()
...
describe('EnvelopeGroup', () => {
  it('renders type label and total', () => {
    renderGroup()
    const header = screen.getByRole('button', { name: /ХаниМани/ })
  ...
  it('toggles collapse on header click', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /ХаниМани/ }))
