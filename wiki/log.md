...
## [2026-09-11] fix | NL-парсер: несколько позиций через запятую

- Баг: «89 flash up, 120 gorilla mango» считалась как 89₽ (всё после первой суммы шло в описание). Формат ХаниМани — позиции через запятую: «3*150 кофе, 2*200 пирожки»
- `parseTransactionInput`: разбивка на позиции по запятым, сумма всех позиций; запятая-десятичный разделитель («1,5*100») распознаётся, когда левый сегмент — сумма без описания; скобки-комментарии игнорируются в каждой позиции
- Тесты: 249 unit (+6: суммы позиций, умножения в позициях, позиция без суммы, скобки, пустые сегменты), e2e без изменений

## [2026-09-11] ux | Клик по строке → действие по умолчанию, читаемая иконка

- Карточки счетов, конвертов и транзакций: клик по строке открывает редактирование (действие по умолчанию), `cursor: pointer` на строках
- Кнопки ✏️/🗑 в поддереве `stopPropagation` — клик по удалению не открывает редактирование
- Иконка «карандаш» заменена: нечитаемый `✎` → эмодзи `✏️` (все три карточки + CategoriesModal)
- Тесты: 243 unit (+5: клик по строке, клик по удалению не редактирует), 37 e2e — стабильны в 3 прогонах
